...
% qua descrivi gli strumenti di misura e come hai preso i dati. ricavati le formule che usi in questa sezione. non mette i numeri, vengono dopo
	\section{Analisi dei dati}
%qua ti calcoli cosa ti devi calcolare. alla fine di questa sezione ti trovi il risultato e la sua insicurezza. è in questa sezione che ci metti grafici, regressioni, teorema di gauss per il calcolo dell'errore etc.
...
%qua ci scrivi se i risultati del tuo esperimento coincidono con i valori dei tuoi riferimenti bibliografici. quando lo paragoni a qualcosa mettici sempre la fonte, tipo il libro (autore: titolo, editore, luogo, anno) o il sito (mettici ora e data di quando lo controlli con una nota a piè di pagina) esempio:
...
%qua a noi ci fanno mettere lo scan dei dati che abbiamo scritto in laboratorio, in ogni caso, è l'algoritmo per mettere pdf, jpeg, png etc.
			%\begin{figure}[H]
			%	\centering
			%	\includegraphics[width=13cm]{measured_data.pdf}
			%	\caption{Seite 1 der Messwerte}
			%\end{figure}
		%\subsection{Abbildungen}
\subsection{Tabelle}
%ti ce ne metto una per farti un esempio, poi magari scrivimi se la vuoi cambiare
...
%last but not least la bibliografia
	\begin{thebibliography}{1}
		...
		\bibitem{PraktikumsScript}Dr. Uwe Müller: \textit{Physikalisches Grundpraktikum: Einführung in die Messung, Auswertung und Darstellung experimenteller Ergebnisse in der Physik}, Berlin, 2007
	\end{thebibliography}
\end{document}
